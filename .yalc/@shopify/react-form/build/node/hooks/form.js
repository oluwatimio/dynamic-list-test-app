...
function useForm({
  ...
  const dirty = (0, _dirty.useDirty)(fields);
  const basicReset = (0, _reset.useReset)(fields);
  const {
    submit,
    submitting,
    errors,
    setErrors
  } = (0, _submit.useSubmit)(onSubmit, fields, makeCleanAfterSubmit);
  const reset = (0, _react.useCallback)(() => {
    setErrors([]);
    basicReset();
  }, [basicReset, setErrors]);
  const fieldsRef = (0, _react.useRef)(fields);
  fieldsRef.current = fields;
  const validate = (0, _react.useCallback)(() => {
    return (0, _utilities.validateAll)(fieldsRef.current);
  }, [fieldsRef]);
  const makeClean = (0, _react.useCallback)(() => (0, _utilities.makeCleanFields)(fieldsRef.current), [fieldsRef]);
  return {
    fields,
    dirty,
    submitting,

...
function useField(input, dependencies = []) {
  ...
  const validators = (0, _utilities.normalizeValidation)(validates);
  const [state, dispatch] = (0, _reducer.useFieldReducer)(value, dirtyStateComparator);
  const resetActionObject = (0, _react.useMemo)(() => (0, _reducer.resetAction)(), []);
  const reset = (0, _react.useCallback)(() => dispatch(resetActionObject), [dispatch, resetActionObject]);
  const newDefaultValue = (0, _react.useCallback)(value => dispatch((0, _reducer.newDefaultAction)(value)), [dispatch]);
  const runValidation = (0, _react.useCallback)((value = state.value) => {
    const errors = validators.map(check => check(value, {})).filter(value => value != null);

    if (errors && errors.length > 0) {
      const [firstError] = errors;
      dispatch((0, _reducer.updateErrorAction)(errors));
      return firstError;
    }

    dispatch((0, _reducer.updateErrorAction)(undefined));
  ...
  const onChange = (0, _react.useCallback)(value => {
    ...
    dispatch((0, _reducer.updateAction)(normalizedValue));

    if (state.error) {
      runValidation(normalizedValue);
    }
  }, [runValidation, state.error, dispatch]);
  const setError = (0, _react.useCallback)(value => dispatch((0, _reducer.updateErrorAction)(value)), [dispatch]);
  ...
  (0, _react.useEffect)(() => {
    if (!(0, _fastDeepEqual.default)(value, state.defaultValue)) {

...
import { validateAll, makeCleanFields } from "../utilities.mjs";
...
export function useForm(_ref) {
  ...
  var dirty = useDirty(fields);
  var basicReset = useReset(fields);

  var _useSubmit = useSubmit(onSubmit, fields, makeCleanAfterSubmit),
  ...
  var makeClean = useCallback(function () {
    return makeCleanFields(fieldsRef.current);
  }, [fieldsRef]);
  return {
    fields: fields,
    dirty: dirty,
